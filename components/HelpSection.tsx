      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {items.map((item) => (
            <div
              key={item.title}
              className="rounded-[2rem] border border-white/70 bg-white/60 p-8 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
            >
              <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#d2f5ef] text-[#4a7d8f]">
                <item.icon size={28} />
              </div>
              ...
              <p className="leading-7 text-[#5a7076]">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
